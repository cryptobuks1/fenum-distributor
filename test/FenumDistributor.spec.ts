...
import Distributor from '../build/FenumDistributor.json'
  ...
  describe('#claim', () => {
    it('fails for empty proof', async () => {
      ...
      await expect(distributor.claim(0, wallet0.address, 10, [])).to.be.revertedWith(
        'FenumDistributor: Invalid proof.'
      )
    })
    ...
    describe('two account tree', () => {
      ...
      it('cannot allow two claims', async () => {
        ...
        await distributor.claim(0, wallet0.address, 100, proof0, overrides)
        await expect(distributor.claim(0, wallet0.address, 100, proof0, overrides)).to.be.revertedWith(
          'FenumDistributor: Drop already claimed.'
        )
      })
      ...
      it('cannot claim more than once: 0 and then 1', async () => {
        ...
        ).to.be.revertedWith('FenumDistributor: Drop already claimed.')
      })
      ...
      it('gas', async () => {
        ...
        expect(receipt.gasUsed).to.eq(78263)
      })
    })
    describe('larger tree', () => {
      ...
      it('gas', async () => {
        ...
        const receipt = await tx.wait()
        expect(receipt.gasUsed).to.eq(80757)
      })
      ...
      it('gas second down about 15k', async () => {
        ...
        const receipt = await tx.wait()
        expect(receipt.gasUsed).to.eq(65737)
      })
    })
    ...
    describe('realistic size tree', () => {
      ...
      it('gas', async () => {
        ...
        const receipt = await tx.wait()
        expect(receipt.gasUsed).to.eq(91447)
      })
      it('gas deeper node', async () => {
        ...
        const receipt = await tx.wait()
        expect(receipt.gasUsed).to.eq(91383)
      })
